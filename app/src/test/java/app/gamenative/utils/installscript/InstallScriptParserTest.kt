...
class InstallScriptParserTest {
    ...
    @Test
    fun parseRegistry_expandsRootDriveWithColon() {
        val vdf = """
            "InstallScript"
            {
                "Registry"
        ...
        val result = InstallScriptParser.parseFromString(vdf, installDir)
        assertEquals("C:\\GameData", result.registryActions[0].values.strings["DataDir"])
    }
    ...
    @Test
    fun parse_returnsEmptyForMalformedContent() {
        val result = InstallScriptParser.parseFromString("not a vdf {{{", installDir)
        assertTrue(result.registryActions.isEmpty())
        assertTrue(result.runProcessActions.isEmpty())
    }
}
