...
/**
 * KGSL's `gpubusy` reports cumulative busy and total ticks, not an instant
 * percentage. Only their delta between two HUD samples is a meaningful load.
 */
internal fun kgslGpuUsagePercent(
    previousBusy: Long?,
    previousTotal: Long?,
    busy: Long,
    total: Long,
): Int? {
    if (previousBusy == null || previousTotal == null) return null
    val busyDelta = busy - previousBusy
    val totalDelta = total - previousTotal
    if (busyDelta < 0L || totalDelta <= 0L) return null
    return ((busyDelta.coerceAtMost(totalDelta) * 100L) / totalDelta)
        .toInt()
        .coerceIn(0, 100)
}

/**
 * Some Snapdragon kernels deliberately expose zeroes through `gpubusy` to
 * applications, while `gpu_clock_stats` still accounts for time spent at each
 * active power level.  The entries are cumulative milliseconds, so their sum
 * over the HUD sampling interval is a usable last-resort activity signal.
 */
internal fun kgslClockResidencyUsagePercent(
    previousResidencyMs: Long?,
    previousWallMs: Long?,
    residencyMs: Long,
    wallMs: Long,
): Int? {
    if (previousResidencyMs == null || previousWallMs == null) return null
    val residencyDelta = residencyMs - previousResidencyMs
    val wallDelta = wallMs - previousWallMs
    if (residencyDelta < 0L || wallDelta <= 0L) return null
    return ((residencyDelta * 100L) / wallDelta)
        .toInt()
        .coerceIn(0, 100)
}
...
class PerformanceHudView(
    ...
    // Qualcomm KGSL exposes cumulative busy/total ticks in `gpubusy`.
    private var lastKgslGpuBusy: Long? = null
    private var lastKgslGpuTotal: Long? = null

    // Qualcomm's gpu_clock_stats is a list of cumulative residency values,
    // one for each GPU power level.
    private var lastKgslClockResidencyMs: Long? = null
    private var lastKgslClockResidencyWallMs: Long? = null
    ...
    private fun readGpuUsagePercent(): Int? {
        val readings = discoverGpuUsagePaths()
            .asSequence()
            .mapNotNull { readGpuUsageSample(it) }
            .toList()

        // A frozen KGSL counter often returns zero even when the clock
        // residency fallback sees activity. Prefer a real non-zero sample,
        // then preserve zero for genuinely idle GPUs.
        return readings.firstOrNull { it.percent > 0 }?.percent
            ?: readings.firstOrNull()?.percent
    }
    ...
    private fun readGpuUsageSample(path: String): GpuUsageReading? {
        ...
        return when (fileName) {
            "gpubusy" -> {
                ...
                val percent = kgslGpuUsagePercent(
                    previousBusy = lastKgslGpuBusy,
                    previousTotal = lastKgslGpuTotal,
                    busy = busy,
                    total = total,
                )
                lastKgslGpuBusy = busy
                lastKgslGpuTotal = total
                percent?.let { GpuUsageReading(it, path) }
            }
            "gpu_clock_stats" -> {
                val residencyMs = readFirstLine(path)
                    ?.trim()
                    ?.split(Regex("\\s+"))
                    ?.sumOf { it.toLongOrNull() ?: 0L }
                    ?: return null
                val now = SystemClock.elapsedRealtime()
                val percent = kgslClockResidencyUsagePercent(
                    previousResidencyMs = lastKgslClockResidencyMs,
                    previousWallMs = lastKgslClockResidencyWallMs,
                    residencyMs = residencyMs,
                    wallMs = now,
                )
                lastKgslClockResidencyMs = residencyMs
                lastKgslClockResidencyWallMs = now
                percent?.let { GpuUsageReading(it, path) }
            }
            "gpuinfo" -> {
