...
import app.gamenative.ui.component.layers.ProvideLayerController
import app.gamenative.ui.component.layers.rememberLayerController
...
import app.gamenative.ui.gcds.GcdsToast
...
import app.gamenative.utils.CommunityFeedbackLinks
...
fun PluviaMain(
    ...
    // Global two-axis layer model (L0 game / L1 shell / L2 quick / L3 system / L4 dialog).
    // Owned here so the system menu can overlay any screen incl. in-game.
    val layerController = rememberLayerController()
    val lifecycleScope = lifecycleOwner.lifecycleScope

    val state by viewModel.state.collectAsStateWithLifecycle()

    var requestDownloads by remember { mutableStateOf(false) }
    ...
    when (msgDialogState.type) {
        ...
        DialogType.CRASH -> {
            ...
            onActionClick = {
                CrashHandler.buildCrashEmailIntent(context)?.let { intent ->
                    context.startActivity(
                        Intent.createChooser(intent, context.getString(R.string.crash_report_send_title)),
                    )
                }
                viewModel.setHasCrashedLastStart(false)
                setMessageDialogState(MessageDialogState(false))
    ...
    // Platform OAuth launchers used by the global system menu (store account rows).
    ...
    val systemIsOffline by viewModel.isOffline.collectAsStateWithLifecycle()
    ...
    PluviaTheme(
        ...
        ProvideLayerController(layerController) {
        Box(modifier = Modifier.fillMaxSize()) {
            ...
            GameFeedbackDialog(
                ...
                onDiscordSupport = {
                    uriHandler.openUri(CommunityFeedbackLinks.DISCORD_INVITE_URL)
                },
                onOpenGithubIssue = { feedbackState ->
                    uriHandler.openUri(CommunityFeedbackLinks.githubIssueUrl(feedbackState))
                },
            )
            ...
            // GCDS 4.3: fade + 16dp axis slide; direction signals travel
            // (push slides in from the end edge, pop returns from the start edge).
            // NavHost transition lambdas are not composable, so the reduced-motion
            // read happens here and plain specs are captured.
            val navMotionMs = 220
            val reduceMotion = LocalReduceMotion.current
            val navFloatSpec: FiniteAnimationSpec<Float> = if (reduceMotion) snap() else tween(navMotionMs)
            val navIntSpec: FiniteAnimationSpec<IntOffset> = if (reduceMotion) snap() else tween(navMotionMs)
            NavHost(
                ...
                composable(
                        ...
                        if (shouldShowDialogs && !state.annoyingDialogShown && PluviaApp.xEnvironment == null && !SteamService.keepAlive && !MainActivity.wasLaunchedViaExternalIntent) {
                            ...
                            if (currentUpdateInfo != null) {
                                ...
                                msgDialogState = MessageDialogState(
                                    ...
                                    actionBtnText = context.getString(R.string.crash_report_send),
                                )
                            }
                        }
                    }
                    ...
                    HomeScreen(
                        ...
                        downloadsRequested = requestDownloads,
                        onDownloadsRequestedHandled = { requestDownloads = false },
                        isOffline = isOffline,
            ...
            if (snackbarController.rootOwnsHost) {
                SnackbarHost(
                    ...
                    Box(
                        ...
                        GcdsToast(message = data.visuals.message)
                    }
                }
            }
            ...
            // Global system menu (L3 overlay) — visible from every screen. The
            // second display owns its own full-screen hub, so skip here when DS.
            if (layerController.isSystemOpen && !hasExternalDisplay) {
                SystemMenu(
                    isOpen = true,
                    ...
                    onNetworkToggle = {
                        viewModel.setOffline(!systemIsOffline)
                        navController.navigate(
                            PluviaScreen.Home.route + "?offline=${!systemIsOffline}",
                        )
                    },
                    ...
                    onNotificationsToggle = {
                        if (!notificationPermissionGranted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                            ...
                            SnackbarManager.show(context.getString(R.string.system_menu_notifications_permission_denied))
                        }
                    },
                    audioEnabled = PrefManager.systemAudioEnabled,
                    onAudioToggle = {
                        PrefManager.systemAudioEnabled = !PrefManager.systemAudioEnabled
                    },
                    onExitGame = {
                        layerController.closeSystem()
                        if (state.launchedAppId != null) {
