...
internal class RadialMenuOverlayView(context: Context) : View(context) {
    ...
    private fun drawSlotLabel(
        ...
        val label = fitLabel(localizedSlotLabel(slot).take(18), maxLabelWidth)
        canvas.drawText(label, textX, centeredTextBaseline(textY, textPaint), textPaint)
    }

    private fun localizedSlotLabel(slot: RadialMenu.Slot): String {
        if (slot.label.isNotBlank()) return slot.label
        val resource = when (slot.binding) {
            com.winlator.inputcontrols.Binding.KEY_I -> R.string.radial_menu_inventory
            com.winlator.inputcontrols.Binding.KEY_M -> R.string.radial_menu_map
            com.winlator.inputcontrols.Binding.KEY_J -> R.string.radial_menu_journal
            com.winlator.inputcontrols.Binding.KEY_F5 -> R.string.radial_menu_quick_save
            com.winlator.inputcontrols.Binding.KEY_F9 -> R.string.radial_menu_quick_load
            com.winlator.inputcontrols.Binding.KEY_ESC -> R.string.radial_menu_escape
            com.winlator.inputcontrols.Binding.KEY_TAB -> R.string.radial_menu_tab
            com.winlator.inputcontrols.Binding.SHOW_KEYBOARD -> R.string.radial_menu_keyboard
            else -> null
        }
        return resource?.let(context::getString) ?: slot.displayLabel
    }
