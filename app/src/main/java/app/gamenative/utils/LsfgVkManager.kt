 ...
 * 3. Write conf.toml (version 1) with a catch-all [[game]] entry, the DLL path,
 *    multiplier, flow scale, and performance mode. The layer reads it on init.
 * 4. Set env vars so the layer finds its config. The catch-all entry is scoped
 *    to this container because the layer itself is installed per container.
 */
object LsfgVkManager {
    ...
    private const val BROKEN_V2_MANIFEST_FILENAME = "VkLayer_LSFGVK_frame_generation.json"
    private const val VERSION_FILENAME = ".lsfg_vk_runtime_version"
    private const val GENERATED_MARKER_RELATIVE_PATH = "usr/tmp/lsfg-vk_generated"
    ...
    // its exact historical value is required by the bundled Android layer.
     ...
     * presented at least one intermediary frame during the current launch.
     */
    @JvmStatic
    fun hasPresentedGeneratedFrame(container: Container): Boolean =
        File(container.rootDir, GENERATED_MARKER_RELATIVE_PATH).isFile
    ...
    fun ensureRuntimeInstalled(context: Context, container: Container): Boolean {
        ...
        val brokenV2Manifest = File(layerDir, BROKEN_V2_MANIFEST_FILENAME)
        ...
        // dev40 can block forever in its first present on Android/Turnip. Its
        // manifest has a different name, so merely installing the recovered
        // runtime would otherwise leave both Vulkan layers discoverable.
        if (brokenV2Manifest.exists()) brokenV2Manifest.delete()
     ...
     *
     * The bundled native parser requires version = 1 and [[game]]. The layer is
     * installed per container, so an empty executable suffix intentionally
     * matches every Vulkan process in that container. Multiplier 1 is its
     * supported passthrough mode for runtime toggling.
     *
    ...
    fun applyLaunchEnv(container: Container, envVars: EnvVars): Boolean {
        ...
        // Older Gameplay builds set this variable, but the bundled native
        // parser never consumed these. Remove them so stale launch environments
        // do not imply that config/profile selection is working.
        envVars.remove("LSFGVK_CONFIG")
        envVars.remove("LSFGVK_PROFILE")
        // The native layer recreates this only after an intermediary frame has
        // actually reached vkQueuePresentKHR in the new game session.
        File(container.rootDir, GENERATED_MARKER_RELATIVE_PATH).delete()
    ...
    private fun disableLayerInContainer(container: Container) {
        ...
        listOf(MANIFEST_FILENAME, BROKEN_V2_MANIFEST_FILENAME).forEach { name ->
            val manifest = File(layerDir, name)
            if (manifest.exists()) {
                manifest.delete()
                Timber.tag(TAG).d("Removed %s to disable LSFG", name)
            }
        }
    }
     ...
     * swapchain.
     *
     * The config is always rewritten with a [[game]] entry: multiplier >= 2
    ...
    fun updateConfigAtRuntime(
        ...
        return try {
            // multiplier == 1 keeps a valid game entry while expressing "off".
            ...
            if (ok) {
                Timber.tag(TAG).i(
                    ...
                    enabled, effectiveMultiplier, flowScale, performanceMode && enabled
                )
            }
            ok
        } catch (t: Throwable) {
